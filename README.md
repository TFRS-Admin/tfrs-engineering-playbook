...
- **This repository** defines *what TFRS requires and how work is tracked*: standards, the lifecycle, issue Metadata conventions.
...
| Issue Metadata (Status, Priority, Risk, Size, Epic, Sprint, Blocked, QA Required, Agent Persona) | [`ISSUE_METADATA_STANDARD.md`](./ISSUE_METADATA_STANDARD.md) — the operational source of truth for issue state |
| GitHub Projects (optional dashboard) | [`GITHUB_PROJECT_STANDARD.md`](./GITHUB_PROJECT_STANDARD.md) — never required for operation |
| Standards and thresholds (PR size, review bar, security, testing, sizing) | The relevant `*_STANDARD.md` — [`REVIEW_STANDARD.md`](./REVIEW_STANDARD.md), [`EXECUTION_STANDARD.md`](./EXECUTION_STANDARD.md), [`PLANNING_STANDARD.md`](./PLANNING_STANDARD.md), [`SECURITY_STANDARD.md`](./SECURITY_STANDARD.md), [`TESTING_STANDARD.md`](./TESTING_STANDARD.md), [`BACKLOG_STANDARD.md`](./BACKLOG_STANDARD.md), [`REPO_HEALTH_STANDARD.md`](./REPO_HEALTH_STANDARD.md) |
| Merge autonomy (when an agent may merge automatically vs. wait for approval) | [`PR_AUTONOMY_STANDARD.md`](./PR_AUTONOMY_STANDARD.md) |
...
2. **Create the repository engineering documentation** — `ARCHITECTURE.md` and the four `docs/engineering/` files (`ROADMAP.md`, `BACKLOG.md`, `CURRENT_SPRINT.md`, `REPO_HEALTH.md`), seeded from the templates in [`templates/README.md#repository-engineering-docs`](./templates/README.md#repository-engineering-docs). This — not a GitHub Project — is what makes the repository operational.
...
4. Follow the full onboarding lifecycle in [`REPOSITORY_BOOTSTRAP_GUIDE.md`](./REPOSITORY_BOOTSTRAP_GUIDE.md) — it walks through repository creation, playbook adoption, establishing repository engineering documentation, and the first pass through Review → Plan → Backlog → Execute, using `tfrs-website`'s real, current issue hierarchy as a worked example — including where that repository's own adoption is still incomplete, per its [Repository Readiness Checklist](./REPOSITORY_BOOTSTRAP_GUIDE.md#repository-readiness-checklist) result.
5. Record the adoption decision in your repository docs. A GitHub Project is optional — see [`GITHUB_PROJECT_STANDARD.md`](./GITHUB_PROJECT_STANDARD.md) — and never gates this step.
...
- **Backlog** converts plans into Epics and Issues (each carrying a `## Metadata` block) and repository engineering documentation — the phase this version of the playbook adds ([`commands/backlog.md`](./commands/backlog.md), [`BACKLOG_STANDARD.md`](./BACKLOG_STANDARD.md)).
...
- [ISSUE_METADATA_STANDARD.md](./ISSUE_METADATA_STANDARD.md)
...
- [PR_AUTONOMY_STANDARD.md](./PR_AUTONOMY_STANDARD.md)
...
- [templates/project-board-template.md](./templates/project-board-template.md) (optional)
...
- [templates/engineering-backlog-template.md](./templates/engineering-backlog-template.md)
- [templates/engineering-current-sprint-template.md](./templates/engineering-current-sprint-template.md)
- [templates/engineering-repo-health-template.md](./templates/engineering-repo-health-template.md)
...
| --- | --- | --- |
| tfrs-website | Not Onboarded (2 of 14 on the [Repository Readiness Checklist](./REPOSITORY_BOOTSTRAP_GUIDE.md#repository-readiness-checklist); 10 non-degradable failures) | Has a real, dependency-mapped issue hierarchy and runnable verification commands, used as this playbook's worked example — but its own `AGENTS.md` still points at a vendored skills copy instead of this playbook, and it's missing `CLAUDE.md`, `AI_AGENT_OPERATING_MODEL.md`, `DECISION_ROUTER.md`, `ARCHITECTURE.md`, `docs/engineering/`, `.github/ISSUE_TEMPLATE/`, `.github/PULL_REQUEST_TEMPLATE.md`, and any playbook/skills-repo reference. See the checklist result for the full breakdown before treating it as onboarded at all. |
...
- **Create (GitHub-native):** `.github/ISSUE_TEMPLATE/` (carrying the `## Metadata` block from [`ISSUE_METADATA_STANDARD.md`](./ISSUE_METADATA_STANDARD.md)), `.github/PULL_REQUEST_TEMPLATE.md` — must live in the consuming repository to take effect.
- **Create (repository engineering documentation):** `ARCHITECTURE.md` and the four `docs/engineering/` files (`ROADMAP.md`, `BACKLOG.md`, `CURRENT_SPRINT.md`, `REPO_HEALTH.md`), seeded from [`templates/README.md#repository-engineering-docs`](./templates/README.md#repository-engineering-docs). This is the repository-centered replacement for GitHub Project setup — see [Architecture](#architecture) above.
...
`REVIEW_STANDARD.md` and `EXECUTION_STANDARD.md` are **not** part of the baseline, despite being described that way in earlier guidance — they belong in [What Should Stay Centralized](#what-should-stay-centralized) below, alongside every other `*_STANDARD.md` document. A GitHub Project (see [`GITHUB_PROJECT_STANDARD.md`](./GITHUB_PROJECT_STANDARD.md)) is optional and is not part of this baseline in any form.
...
- [`REVIEW_STANDARD.md`](./REVIEW_STANDARD.md), [`PLANNING_STANDARD.md`](./PLANNING_STANDARD.md), [`EXECUTION_STANDARD.md`](./EXECUTION_STANDARD.md), [`BACKLOG_STANDARD.md`](./BACKLOG_STANDARD.md), [`ISSUE_METADATA_STANDARD.md`](./ISSUE_METADATA_STANDARD.md), [`GITHUB_PROJECT_STANDARD.md`](./GITHUB_PROJECT_STANDARD.md), [`SECURITY_STANDARD.md`](./SECURITY_STANDARD.md), [`TESTING_STANDARD.md`](./TESTING_STANDARD.md), [`SKILLS_STANDARD.md`](./SKILLS_STANDARD.md), [`REPO_HEALTH_STANDARD.md`](./REPO_HEALTH_STANDARD.md), [`PR_AUTONOMY_STANDARD.md`](./PR_AUTONOMY_STANDARD.md) — the standards themselves. (`DECISION_ROUTER.md` is the one exception among the all-caps root documents — it's part of the copy baseline, not this list, per [`commands/setup-from-playbook.md#minimum-baseline`](./commands/setup-from-playbook.md#minimum-baseline).)
...
Versioning rules and the full changelog live in [`VERSION.md`](./VERSION.md) — that is the single authoritative source; this section only states how the strategy affects adoption. In short: `major` changes require downstream repositories to adjust (e.g. adding new required issue Metadata fields or repository engineering documents); `minor` changes add capability without breaking anything already adopted; `patch` changes are safe to ignore until convenient.

### Upgrade Strategy

1. Watch this repository (or the `playbook_improvement` issue label) for new entries in [`VERSION.md`](./VERSION.md).
2. On a `major` bump, treat the upgrade as its own piece of work: run the relevant step of [`REPOSITORY_BOOTSTRAP_GUIDE.md`](./REPOSITORY_BOOTSTRAP_GUIDE.md) again (e.g. re-run the Repository Readiness Checklist to confirm new required files exist) rather than assuming the old configuration still satisfies the standard.
...
2. Plan the change against [`PLANNING_STANDARD.md`](./PLANNING_STANDARD.md) and, if it touches issue state, [`ISSUE_METADATA_STANDARD.md`](./ISSUE_METADATA_STANDARD.md).
