...
8. **The relevant skill in [`TFRS-Admin/agent-skills`](https://github.com/TFRS-Admin/agent-skills)** — once the task type is known, per [`SKILLS_STANDARD.md#skill-selection`](./SKILLS_STANDARD.md#skill-selection).
...
3. **Highest `Priority`**, then highest `Risk` (resolve risk while there is runway in the sprint), then smallest `Size` when priority and risk tie (see [`BACKLOG_STANDARD.md#execution-ordering`](./BACKLOG_STANDARD.md#execution-ordering)).
...
| `Backlog-Manager` | Runs [`commands/backlog.md`](./commands/backlog.md) — converts a plan into Epics, Issues, and repository engineering documentation, dependency-ordered for execution. |
...
- [`GITHUB_PROJECT_STANDARD.md`](./GITHUB_PROJECT_STANDARD.md) — the optional dashboard layer, consulted only if a repository runs one
