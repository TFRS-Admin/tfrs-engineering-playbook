...
- **This checklist is the approval bar every PR clears before merge — it does not by itself say who performs the merge action, or whether a human needs to be in that final step.** [`PR_AUTONOMY_STANDARD.md`](./PR_AUTONOMY_STANDARD.md) answers that, once this checklist and [`commands/verify.md`](./commands/verify.md) both pass: low-risk, narrowly-scoped changes (documentation, dependency patches, tests, CI, styling, repository adoption and engineering-doc updates, issue Metadata bookkeeping) merge automatically at `Level 1`; changes touching auth, pricing, permissions, business logic, API contracts, schema, infrastructure, architecture, or large UI surfaces wait for explicit approval at `Level 2`; the highest-risk categories (secrets, security incidents, emergency rollback, destructive migrations, legal/compliance) are `Level 3` and are never merged by an agent at all. A `Risk: High`/`Risk: Critical` issue reviewed under this standard is very unlikely to resolve to `Level 1` — but `PR_AUTONOMY_STANDARD.md`'s classification of the actual diff, not this document's review outcome alone, is what [`commands/ship.md`](./commands/ship.md) checks before merging.
...
This document is the approval bar for a specific pull request's diff. For assessing the *current state* of a system or feature area before planning new work — not a diff — see [`commands/review.md`](./commands/review.md), which produces findings rather than approve/reject decisions; both documents are named "Review" but govern opposite ends of the lifecycle — see the terminology map in [`AI_ENGINEERING_WORKFLOW.md`](./AI_ENGINEERING_WORKFLOW.md#terminology-map) if that's ambiguous. See also [`commands/verify.md`](./commands/verify.md) for the evidence a PR must carry before it reaches this review bar, [`PR_AUTONOMY_STANDARD.md`](./PR_AUTONOMY_STANDARD.md) for who performs the merge once this bar and verification are both cleared, [`SECURITY_STANDARD.md`](./SECURITY_STANDARD.md) and [`TESTING_STANDARD.md`](./TESTING_STANDARD.md) for the full standards behind the Security and Tests axes above, and [`REPO_HEALTH_STANDARD.md`](./REPO_HEALTH_STANDARD.md) for recurring, cadence-driven assessment.
